...
const DiemDanhController = require("./controller/DiemDanh");
...
var storageimg = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(
      null,
      `./public/labeled_images/${req.session.ClassID}/${FolderStudentID}`
    );
  },
  filename: function (req, file, cb) {
    cb(null, "1.png");
...
const checkSession = (req, res, next) => {
  if (req.session.TeacherID == null && req.session.StudentID == null) {
    return res.send(
      '<script>alert("Phiên làm việc đã hết,vui lòng đăng nhập lại")</script>'
    );
  }
...
app.get("/deleteClass/:id", (req, res) => {
  Class.findById(req.params.id, function (error, detailClass) {
    ...
    ClassID = req.session.ClassID;
    console.log(ClassID);
    BlogPost.deleteMany({ ClassID: ClassID })
      .then((response) => {
    ...
    Diem.deleteMany({ ClassID: ClassID })
      .then((response) => {
        console.log(response);
      })
      .catch((err) => {
    ...
    Homework.deleteMany({ ClassID: ClassID })
      .then((response) => {
        console.log(response);
      })
      .catch((err) => {
    ...
    Student.deleteMany({ ClassID: ClassID })
      .then((response) => {
        console.log(response);
      })
      .catch((err) => {
    ...
    fs.rmdirSync(`./public/labeled_images/${req.session.ClassID}`, {
      recursive: true,
    });
    res.redirect("/viewClass");
  });
...
app.post("/diem", (req, res) => {
  Diem.findOne(
    { StudentID: req.session.StudentID, ClassID: req.session.ClassID },
    function (error, sldiem) {
      if (sldiem) {
        ...
        if (req.body.CuoiKy) {
          Diem.findOneAndUpdate(
            { StudentID: req.session.StudentID, ClassID: req.session.ClassID },
            {
              $set: {
                GiuaKy: arr,
                Tong: Number(Tong),
                CuoiKy: req.body.CuoiKy,
              },
            },
            function (err, data) {
              res.render("pgTeacher");
          ...
          Diem.findOneAndUpdate(
            { StudentID: req.session.StudentID, ClassID: req.session.ClassID },
            { $set: { GiuaKy: arr, Tong: Number(Tong) } },
            function (err, data) {
              res.render("pgTeacher");
            }
          );
        ...
        Diem.create(
          {
            GiuaKy: req.body.GiuaKy,
            CuoiKy: req.body.CuoiKy,
            Tong: Number(req.body.GiuaKy) / 2 + Number(req.body.CuoiKy) / 2,
...
app.get("/xemdiem", (req, res) => {
  Diem.find(
    { ClassID: req.session.ClassID, StudentID: req.session.StudentID },
    function (error, xemdiem) {
      //console.log(req.session);
...
app.get("/homework", (req, res) => {
  if (TeacherloggedIn == null) {
    ...
    Homework.find(
      { ClassID: req.session.ClassID, TeacherID: req.session.TeacherID },
      (err, data) => {
        res.render("homework", { data: data });
      }
    );
...
app.get("/delete/student", (req, res) => {
  Student.findOneAndDelete(
    { Email: req.session.StudentMail, ClassID: req.session.ClassID },
    function (err, docs) {
      if (err) {
        console.log(err);
      } else {
...
io.on("connection", (socket) => {
  socket.emit("name-id", ClassofID, Name, FolderStudentID);
  socket.on("reckonized", async (tenhs) => {
		// const session = await mongoose.startSession();
    // session.startTransaction(); //start transaction
    // const opts = { session, new: true };
    try {
			Tentrave = tenhs;
			console.log("tên:",Tentrave)
      ...
      const toDay =
        ...
        currentdate.getSeconds();
			const email = Tentrave.split(" ");
			console.log("email: ",email[0])
      const students = await Student.findOne({ Email: email[0] });
			console.log("student: ",students);
			console.log("ClassID: ",students.ClassID);
      const classId = await Class.findOne({ _id: students.ClassID });
			console.log("class: ",classId)
			const existedDiemDanh = await DiemDanh.find(
        {
          Ngay: toDay,
          ClassID: classId.Name,
          TenSinhVien: students.Name,
          StudentID: email[0],
          TenLop: classId.Name,
          MaLop: students.ClassID,
          MaGV: students.StudentID,
        }
      );
			console.log("check:",existedDiemDanh);
			if(!existedDiemDanh.length){
				const diemdanh = await DiemDanh.create(
          {
            Ngay: toDay,
            ClassID: classId.Name,
            TenSinhVien: students.Name,
  ...
  socket.on("join-room", async (roomId, userId) => {
    ...
    if (TeacherloggedIn == null)
      socket
        .to(roomId)
        .broadcast.emit("user-connected", formatUserId(Name, userId));
    else
      socket
        .to(roomId)
        .broadcast.emit("user-connected", formatUserId(msTeacherName, userId));
    console.log("UserId", userId);
    socket.on("message", (message) => {
      if (TeacherloggedIn == null) {
        io.to(roomId).emit(
          "createMessage",
          formatMessages(Name, message),
          userId
        );
      } else {
        io.to(roomId).emit(
          "createMessage",
          formatMessages(msTeacherName, message),
          userId
        );
      }
    });
    socket.on("disconnect", () => {
      if (TeacherloggedIn == null)
        socket
          .to(roomId)
          .broadcast.emit("user-disconnected", formatUserId(Name, userId));
      else
        socket
          .to(roomId)
          .broadcast.emit(
            "user-disconnected",
            formatUserId(msTeacherName, userId)
          );
    });
  });
});
...
passport.use(
  new LocalStrategy(
    ...
    function (Email, Password, done) {
      Student.findOne({ Email: Email }).then(function (user) {
